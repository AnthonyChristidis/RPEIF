...
#'
IF.ES <- function(returns=NULL, evalShape=FALSE, retVals=NULL, parsNuisance.IF=NULL, k=4,
  ...
  if(isTRUE(evalShape)){
    if(is.null(retVals))
      if(!is.null(returns))
        ...
        IFvals <- cbind(retVals, IF.fn(retVals, risk="ES", returns, parsNuisance.IF, alpha.ES))
