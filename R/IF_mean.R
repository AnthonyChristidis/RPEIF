...
#' outIF <- IF.mean(returns=NULL, evalShape=TRUE, retVals=NULL, parsNuisance.IF=NULL,
...
#'
IF.mean <- function(returns=NULL, evalShape=FALSE, retVals=NULL, parsNuisance.IF=NULL, k=4,
    ...
    if(is.null(retVals))
      if(!is.null(returns))
        retVals <- seq(mean(returns)-k*sd(returns), mean(returns)+k*sd(returns), by=0.001) else
          retVals <- seq(parsNuisance.IF$mu-k*0.07, parsNuisance.IF$mu+k*0.07, by=0.001)
        IFvals <- cbind(retVals, IF.fn(retVals, risk="mean", returns, parsNuisance.IF))
